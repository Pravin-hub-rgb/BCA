...
export class GitManager {
  ...
  init() {
    this.updateCommitInfo();
    setTimeout(() => this.updateCommitInfo(), 1000);
    // Refresh every 10 minutes instead of checking constantly
    setInterval(() => this.updateCommitInfo(), 10 * 60 * 1000);
    window.updateCommitInfo = this.updateCommitInfo.bind(this);
  }
  ...
  updateCommitInfo() {
    ...
    const urls = {
      ...
      count: `${this.dataPath}/total-commits.txt?t=${timestamp}`,
      timestamp: `${this.dataPath}/last-updated.txt?t=${timestamp}`,
      dataTimestamp: `${this.dataPath}/data-updated-at.txt?t=${timestamp}`
    };
    ...
    this.testAndFetchCount(urls.count);
    this.testAndFetchTimestamp(urls.timestamp);
    this.testAndFetchDataTimestamp(urls.dataTimestamp);
  }
  ...
  // New: Show when data was last refreshed
  async testAndFetchDataTimestamp(url) {
    try {
      const response = await fetch(url, { 
        headers: { 'Cache-Control': 'no-cache', 'Pragma': 'no-cache' } 
      });
      if (!response.ok) return; // Optional field
      const date = new Date((await response.text()).trim());
      if (!isNaN(date.getTime())) {
        const dataRefreshedElement = document.getElementById("data-refreshed-time");
        if (dataRefreshedElement) {
          const timeAgo = this.formatTimeAgo(new Date() - date);
          dataRefreshedElement.textContent = `Data refreshed ${timeAgo}`;
        }
      }
    } catch {
      // Silently fail for optional data timestamp
    }
  }
  ...
  updateCommitMessages(commits) {
    ...
    // Since workflow now filters out automated commits, we can show all commits
    ...
    commitsToShow.forEach((commit, index) => {
      try {
        ...
        // Clean up the message (first line only)
